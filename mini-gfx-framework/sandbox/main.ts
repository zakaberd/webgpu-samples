...
async function main() {
  ...
  if (!canvas) {
    throw new Error('Canvas element with id "gfx" was not found.');
  }
  ...
  const frame = () => {
    ...
    updateCubeFaceText(cubeFaceTransforms, faceTexts, modelMatrix, reusableTextMatrix);
    ...
    const depthAttachment: GPURenderPassDepthStencilAttachment | undefined = frameContext.depthView
      ? {
...
function createFaceTexts(renderer: MsdfTextRenderer, font: MsdfFont): MsdfText[] {
  return [
...
main().catch((error) => {
  console.error('[mini-gfx] initialization failed:', error);
});
